...
while (1 eq 1) {
    ...
    while(my $line = <$info>) {
        ...
        # Fetch shuffle mode
        if ($line =~ m/shuffle_mode=/) {
            # Store break flag
            $shuffle_mode = $line;

            # Format result
            $shuffle_mode =~ s/shuffle_mode=//gi;
            chomp($shuffle_mode);
        }
    }
    ...
    while ($playing eq 0 && $alarm_on eq 0 && $play_music eq 1) {
        ...
        if ($valid_folder eq 0) {
            ...
            if (index($random_folder, "/.") != -1) {
                ...
                # If shuffle mode is songs only
                if ($shuffle_mode eq "songs") {
                    # Play song
                    ...
                    # Generate output
                    $output = "$datestring: $random_folder\n";

                    # Open file for writing
                    open(my $fh, ">>", "/root/albums_listened_to.txt") or die "Could not open file '$file' $!";
